...
- El Worker expone `/api/v1/health` y `/api/v1/health/data`; ambos devuelven 200 cuando el manifest completo de transferencias en R2 está disponible. La proyección consultable usa el binding dedicado `TRANSFERS_DB` (`cambiometro-transferencias`), mientras la base principal conserva las demás entidades. `d1Consistent` compara la proyección dedicada con R2; si una actualización queda incompleta, `transferSource` vuelve explícitamente a `r2` y la API conserva paginación, filtros y checksum.
- La auditoría del lake local encontró una diferencia de frescura, no IDs duplicados: el snapshot versionado `source-health.json` (21-ago) registra 59.361 filas y `$5.011.094.170.302`, mientras el lake completo regenerado el 24-ago contiene 59.544 IDs únicos y `$5.013.581.357.467` (183 filas nuevas, `$2.487.187.165`). El workflow de Pages hidrata el lake completo y regenera la proyección antes de construir; no se debe mezclar el snapshot antiguo con el release nuevo. El health del Worker compara ambos valores y deja la diferencia explícita en `d1Consistent`; la respuesta sigue siendo saludable cuando el manifest completo de R2 está disponible porque R2 es la fuente canónica de costo cero.
...
- `etl-ley-19862.yml` publica las fuentes y el release completo de transferencias. Resuelve la D1 dedicada `cambiometro-transferencias` antes de materializar; la carga se realiza en staging y sólo se activa con el release validado. Si el plan alcanza su almacenamiento total, registra el diagnóstico y mantiene R2 como fallback canónico; cualquier otro error sigue abortando el ETL.
...
El runner local de Cámara conserva checkpoints por diputado en `%TEMP%\\cambiometro-camara-gastos`. La marca de progreso sólo se considera reutilizable cuando existe también el JSON de resultados del diputado; si una fase posterior falla, el siguiente intento puede reconstruir y publicar el snapshot sin volver a descargar diputados ya completados. Los checkpoints se escriben atómicamente y no se suben a Git. La extracción no debe ejecutarse de nuevo sólo para diagnosticar: primero se revisan el log y el checksum del release remoto.
