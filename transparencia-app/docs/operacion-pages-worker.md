...
- El Worker expone `/api/v1/health` y `/api/v1/health/data`; ambos devuelven 200 cuando el manifest completo de transferencias en R2 está disponible. `d1Consistent` informa si la proyección opcional de D1 coincide; cuando D1 está lleno, `transferSource` queda explícitamente en `r2` y la API conserva paginación, filtros y checksum sin duplicar el dataset ni aumentar costos.
...
- `etl-ley-19862.yml` publica las fuentes y el release completo de transferencias. Intenta materializar la proyección D1, pero si Cloudflare devuelve `Exceeded maximum DB size` (code 7500), registra `skipped_r2_canonical`, adjunta el diagnóstico y mantiene R2 como fuente canónica; cualquier otro error sigue abortando el ETL.
