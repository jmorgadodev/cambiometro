...
const enrichedMovimientos = MOVIMIENTOS_RAW.map(m => {
  ...
  // Normalizar sufijos en todas las propiedades
  const salienteNombre = normalizeText(m.salio?.nombre);
  const entranteNombre = normalizeText(m.entro?.nombre);
  const cargoNormalized = normalizeText(m.cargo);
  const organismoNormalized = normalizeText(m.organismo);
  const motivoTextoNormalized = normalizeText(m.salio?.motivo_texto || m.motivo || "Cambio en la conducción institucional.");

  const salioEnriched = m.salio ? {
    ...m.salio,
    nombre: salienteNombre,
    motivo_texto: motivoTextoNormalized,
    dias_en_cargo: diasEnCargo,
    dias_en_cargo_origen: diasOrigen
  } : undefined;

  const entroEnriched = m.entro ? {
    ...m.entro,
    nombre: entranteNombre
  } : undefined;
  ...
  return {
    ...m,
    cargo: cargoNormalized,
    organismo: organismoNormalized,
    estado,
    salio: salioEnriched,
    entro: entroEnriched,
    dias_en_cargo: diasEnCargo,
    ...
    fechaExacta: true,
    tipo: m.tipo_evento,
    organo: organismoNormalized,
    saliente: salienteNombre || undefined,
    entrante: entranteNombre || undefined,
    motivo: motivoTextoNormalized,
