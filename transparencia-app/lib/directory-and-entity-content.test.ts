...
describe("experiencia visual de personas", () => {
  ...
  it("la página /autoridades redirige permanentemente a /personas?tab=parlamentarios", () => {
    ...
    expect(redirects).toContain("/servicios-publicos/gore-ohiggins/ /servicios-publicos/gobierno-regional-del-libertador-ohiggins/ 301");
    expect(redirects).toContain("/servicios-publicos/org-:slug /servicios-publicos/:slug/ 301");
    expect(redirects).toContain("/servicios-publicos/org-:slug/ /servicios-publicos/:slug/ 301");
    expect(staticRedirects).toContain("organismos_adicionales.json");
    const additionalOrganizations = JSON.parse(readFileSync(resolve("data/raw/transparencia_activa/organismos_adicionales.json"), "utf8"));
    const slugify = (value: string) => value.toLowerCase().normalize("NFD").replace(/\p{Diacritic}/gu, "").replace(/[^a-z0-9\s-]/g, "").trim().replace(/\s+/g, "-").replace(/-+/g, "-");
    const legacyOrganizationIds = additionalOrganizations.filter((organization: { id?: string }) => organization.id?.startsWith("org-"));
    expect(legacyOrganizationIds.length).toBeGreaterThan(0);
    expect(legacyOrganizationIds.every((organization: { id: string; nombre: string }) => organization.id.slice("org-".length) === slugify(organization.nombre))).toBe(true);
    const redirectLines = redirects.trim().split(/\r?\n/);
    const dynamicRedirects = redirectLines.filter((line) => /:[A-Za-z][\w]*/.test(line.split(" ")[0]));
    expect(dynamicRedirects.length).toBeLessThanOrEqual(100);
    expect(redirectLines.length - dynamicRedirects.length).toBeLessThanOrEqual(2000);
    expect(redirectLines.length).toBeLessThanOrEqual(2100);
