...
describe("Tarea 15 - 5 Fixes Críticos Pre-Launch", () => {
  ...
  it("Fix 2: Home y /datos muestran 11 fuentes oficiales + 1 derivada con coherencia numérica", () => {
    const homeContent = readFileSync(join(projectRoot, "app", "page.tsx"), "utf8");
    expect(homeContent).toContain("11 fuentes oficiales + 1 derivada (personal de apoyo parlamentario)");
    expect(homeContent).toContain('href="/fuentes"');

    const datosContent = readFileSync(join(projectRoot, "app", "datos", "page.tsx"), "utf8");
    expect(datosContent).toContain("11 fuentes oficiales + 1 derivada");
  });
