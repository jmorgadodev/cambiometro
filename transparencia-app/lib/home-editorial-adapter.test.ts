...
describe("portada editorial conectada al release", () => {
  ...
  it("centra la votación más reciente y muestra las dos anteriores del mismo corte", () => {
    const latestSenateVotes = buildLatestSenateVotes([
      { id: "r2-11352", kind: "vote", sourceId: "votaciones_senado", occurredAt: "2026-09-23", evidence: { sourceUrl: "https://senado.cl/11352" }, data: { id: "sen-vot-11352", fecha_original: "23-09-2026 19:30:00", descripcion: "Votación 11352", resultado: "Aprobado", total_si: "21", total_no: "0", total_abstencion: "0", votos: [...Array.from({ length: 21 }, (_, id) => ({ id: String(id), opcion_valor: "SI" })), ...Array.from({ length: 6 }, (_, id) => ({ id: `np-${id}`, opcion_valor: "NP" })), ...Array.from({ length: 14 }, (_, id) => ({ id: `pareo-${id}`, opcion_valor: "PAREO" }))] } },
      { id: "r2-11354", kind: "vote", sourceId: "votaciones_senado", occurredAt: "2026-09-23", evidence: { sourceUrl: "https://senado.cl/11354" }, data: { id: "sen-vot-11354", fecha_original: "23-09-2026 19:50:00", descripcion: "Votación 11354", resultado: "Aprobado", total_si: "17", total_no: "0", total_abstencion: "0", votos: [...Array.from({ length: 17 }, (_, id) => ({ id: String(id), opcion_valor: "SI" })), ...Array.from({ length: 10 }, (_, id) => ({ id: `np-${id}`, opcion_valor: "NP" })), ...Array.from({ length: 14 }, (_, id) => ({ id: `pareo-${id}`, opcion_valor: "PAREO" }))] } },
    ...
    const latestInCenter = composeHomeLatestSenateVotes(latestSenateVotes)[1];
    expect(latestInCenter).toMatchObject({
      link: "/votaciones-destacadas/#ultimas-senado",
      sourceUrl: "https://senado.cl/11354",
      votosFavor: 100,
      conteoVotosFavor: 17,
      totalVotosEmitidos: 17,
    });
    expect(latestInCenter.externalLink).not.toBe(true);
  });
