...
describe("automatizacion CPLT nacional", () => {
  ...
  it("no bloquea R2 ni Pages cuando falla el archivo secundario de GitHub Releases", () => {
    ...
    expect(packageJson).toContain('"data:finalize:cplt:r2": "node scripts/merge-cplt-category-artifacts.mjs && node --max-old-space-size=6144 scripts/publish-cplt-projections.mjs --r2"');
