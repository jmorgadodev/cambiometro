...
const evidence = (overrides: Partial<PersonSearchEvidence> = {}): PersonSearchEvidence => ({
  id: "row-1", name: "Vanessa Kaiser Barents-Von Hohenhagen", kind: "remuneracion",
  url: "/remuneraciones-publicas/?q=Kaiser", organization: "Senado", sourceId: "senado", ...overrides,
});

describe("búsqueda agrupada de personas", () => {
  it("agrupa por identificador oficial, no por nombre", () => {
  ...
  it("mantiene separadas las coincidencias nominales de fuentes u organismos distintos", () => {
    const groups = groupPersonSearchEvidence([
