...
/**
 * Group only on canonical IDs. Name-only records stay source/organization
 * scoped so homonyms are never presented as one verified person.
 */
...
export function personSearchPage<T>(rows: T[], page: number, pageSize = 15) {
  ...
  return { items: rows.slice(start, start + safeSize), page: safePage, start, end: Math.min(start + safeSize, rows.length), total: rows.length, totalPages };
}
