 ...
 * on the user-facing module instead of opening the generic entity shell,
 * which has no municipal indicators attached to it.
 */
function municipalitySlug(name: string) {
  const commune = name
    .replace(/^\s*(?:i\.?\s+|ilustre\s+)?municipalidad\s+de\s+/i, "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
function officialFilterKeys(requestUrl: URL, datasetRoot = "funcionarios-v1") {
  const keys: string[] = [];
  const values = [
  ...
  if (datasetRoot === "funcionarios-central-v1" && !keys.includes("tipo:servicio")) {
...
async function listFuncionariosFromR2(requestUrl: URL, env: Env, datasetRoot = "funcionarios-v1") {
  ...
  if (!organism || organism === "Todos") {
    ...
    const filterKeys = officialFilterKeys(requestUrl, datasetRoot);
...
async function search(requestUrl: URL, env: Env) {
  ...
  const data = (rows.results ?? []).map((row) => {
    ...
    return { id: item.id, type, nombre: item.name, url: publicEntityPath(item), ...(item.attributes as JsonRecord) };
  });
