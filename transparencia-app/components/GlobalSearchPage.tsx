...
type ApiResult = { id: string; type: string; nombre: string; url?: string; cargo?: string; organo?: string; periodo?: string | null; monto?: number | null; fuente?: string; fuente_url?: string; region?: string };
...
type EntitiesPayload = { data?: Array<{ id: string; kind: string; name: string; attributes?: Record<string, unknown> }>; meta?: { total?: number; page?: number; totalPages?: number } };
  ...
  try {
    ...
    return {
      rows: rows.map((row) => {
        ...
        const id = String(row.recordId ?? `${sourceId}-${name}-${row.periodo ?? ""}`);
        return {
          id, name, kind: "remuneracion", sourceId,
          ...
          sourceUrl: String(row.fuenteUrl ?? row.urlFuente ?? "") || null,
        } satisfies PersonSearchEvidence;
      }).filter((row) => row.name),
      partial: pages.length > boundedPages.length,
    };
  } catch {
...
function kindCategory(kind: string): Exclude<Category, "all"> {
  ...
  if (["entidad", "organismo", "proveedor"].includes(kind)) return "organizations";
  return "people";
}
...
function mapApiResult(row: ApiResult): PersonSearchEvidence {
  ...
  return {
    ...
    name: row.nombre,
    kind: row.type,
    url: row.url || (category === "payments" ? `/remuneraciones-publicas/?q=${encodeURIComponent(row.nombre)}` : `/personas/${encodeURIComponent(row.id)}`),
...
function mapOfficial(row: Record<string, unknown>): PersonSearchEvidence | null {
  ...
  if (!name) return null;
  const id = String(row.id ?? `${row.sourceScope ?? "funcionarios"}-${name}`);
...
function mapEntity(row: { id: string; kind: string; name: string; attributes?: Record<string, unknown> }): PersonSearchEvidence {
  ...
  return {
    ...
    url: kind === "persona" ? `/personas/${encodeURIComponent(row.id)}` : kind === "municipalidad" ? `/municipalidades/?search=${encodeURIComponent(row.name)}` : `/entidades/${encodeURIComponent(row.id)}`,
...
export default function GlobalSearchPage() {
  ...
  useEffect(() => {
    ...
    // The URL is the source of truth only on first load; later submissions update it.
  ...
  const hasNext = page < pageWindow.totalPages
    || (canLoadEntities && entityPage < entityTotalPages)
    || (canLoadOfficials && officialPage < officialTotalPages);

  async function fetchJson<T>(path: string) {
    const response = await fetch(publicApiUrl(path), { cache: "no-store" });
    if (!response.ok) throw new Error("No fue posible consultar los resultados. Intenta nuevamente.");
  ...
  async function runSearch(value = query) {
    ...
    globalThis.history.replaceState(null, "", `/buscar?${params.toString()}`);
    try {
      const [catalog, officials, entities, paymentIndex] = await Promise.all([
        fetchJson<SearchPayload>(`/api/v1/search?q=${encodeURIComponent(clean)}`).catch(() => null),
        fetchJson<OfficialsPayload>(`/api/v1/funcionarios?scope=all&query=${encodeURIComponent(clean)}&include_zero=true&limit=100&page=1`).catch(() => null),
        fetchJson<EntitiesPayload>(`/api/v1/entities?q=${encodeURIComponent(clean)}&limit=100&page=1`).catch(() => null),
        searchPublishedPaymentPages(clean),
      ]);
      if (!catalog && !officials && !entities && paymentIndex.rows.length === 0) throw new Error("No fue posible consultar los resultados. Intenta nuevamente.");
      const catalogRows = [
        ...(catalog?.data?.autoridades ?? []), ...(catalog?.data?.municipalidades ?? []),
        ...(catalog?.data?.funcionarios ?? []), ...(catalog?.data?.remuneraciones ?? []), ...(catalog?.data?.entidades ?? []),
      ].map(mapApiResult);
      ...
      for (const row of [...catalogRows, ...entityRows, ...officialRows, ...paymentIndex.rows]) {
        const key = `${row.kind}:${row.id}`;
        if (!byId.has(key)) byId.set(key, row);
      }
      setEvidence([...byId.values()]);
      setPaymentIndexPartial(paymentIndex.partial);
      setPartialResults(!catalog || !officials || !entities || paymentIndex.partial);
      setOfficialTotal(Number.isFinite(officials?.meta?.total) ? Number(officials?.meta?.total) : null);
      ...
      setError(reason instanceof Error ? reason.message : "No fue posible consultar los resultados.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function nextPage() {
    const next = page + 1;
    if (next > pageWindow.totalPages && !loadingMore
      && ((canLoadEntities && entityPage < entityTotalPages) || (canLoadOfficials && officialPage < officialTotalPages))) {
      setLoadingMore(true);
      try {
        const loadEntities = canLoadEntities && entityPage < entityTotalPages;
        const nextSourcePage = loadEntities ? entityPage + 1 : officialPage + 1;
        const payload = loadEntities
          ? await fetchJson<EntitiesPayload>(`/api/v1/entities?q=${encodeURIComponent(submittedQuery)}&limit=100&page=${nextSourcePage}`)
          : await fetchJson<OfficialsPayload>(`/api/v1/funcionarios?scope=all&query=${encodeURIComponent(submittedQuery)}&include_zero=true&limit=100&page=${nextSourcePage}`);
        const rows = loadEntities
          ? ((payload as EntitiesPayload).data ?? []).map(mapEntity)
          : ((payload as OfficialsPayload).data ?? []).map(mapOfficial).filter((row): row is PersonSearchEvidence => Boolean(row));
        setEvidence((current) => {
          const keys = new Set(current.map((row) => `${row.kind}:${row.id}`));
          return [...current, ...rows.filter((row) => !keys.has(`${row.kind}:${row.id}`))];
        });
        if (loadEntities) setEntityPage(Number(payload.meta?.page ?? nextSourcePage));
        else setOfficialPage(Number(payload.meta?.page ?? nextSourcePage));
        setPage(next);
      } catch (reason) {
    ...
    <main>
      ...
      <section className="container-main" style={{ paddingBlock: "1.5rem 3rem" }}>
        <form role="search" onSubmit={(event: FormEvent) => { event.preventDefault(); void runSearch(); }} className="remuneration-search-form">
          <label className="remuneration-search__main">Nombre, organismo o cargo<input className="form-input" value={query} onChange={(event) => setQuery(event.target.value)} minLength={2} maxLength={80} autoComplete="off" placeholder="Ej.: Kaiser, Torrealba o Ministerio de Salud" /></label>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: ".5rem", marginBlock: "1rem" }}>
          {([ ["all", "Todo"], ["people", "Personas"], ["payments", "Remuneraciones"], ["municipalities", "Municipalidades"], ["organizations", "Organismos"] ] as Array<[Category, string]>).map(([value, label]) => <button key={value} type="button" className={`btn ${category === value ? "btn-primary" : "btn-secondary"}`} aria-pressed={category === value} onClick={() => { setCategory(value); setPage(1); }}>{label}</button>)}
        </div>
        ...
        {!loading && submittedQuery && !error && <>
          <div className="section-heading"><div><p className="eyebrow">RESULTADOS</p><h2>Coincidencias para “{submittedQuery}”</h2><p>{nf.format(filteredGroups.length)} personas y entidades distintas · {nf.format(recordCount)} coincidencias consultadas{officialTotal !== null ? ` · ${nf.format(officialTotal)} filas coincidentes en nóminas oficiales` : ""}</p>{partialResults && <p>Algunas fuentes no entregaron resultados en esta consulta. Las coincidencias disponibles se mantienen visibles.</p>}{paymentIndexPartial && <p>Hay más coincidencias de remuneraciones; agrega otro nombre, organismo o período para acotar la búsqueda.</p>}</div></div>
          {pageWindow.items.length === 0 ? <div className="empty-state"><strong>No encontramos coincidencias</strong><p>Prueba con otro nombre, apellido, organismo o cargo.</p></div> : <div className="evidence-list">
            {pageWindow.items.map((group) => <article key={group.key}>
              <div><span className="status-label status-label--info">{group.kind === "funcionario" || group.kind === "remuneracion" ? "Remuneración" : group.kind === "municipalidad" ? "Municipalidad" : group.kind === "organismo" ? "Organismo" : group.kind === "proveedor" ? "Proveedor" : "Persona / autoridad"}</span></div>
              ...
              {group.identityConfidence === "source-scoped" && <small>Coincidencia agrupada dentro de la misma fuente y organismo; no implica identidad confirmada entre fuentes.</small>}
              <small>{nf.format(group.evidence.length)} coincidencia{group.evidence.length === 1 ? "" : "s"} en esta búsqueda</small>
              {group.evidence.length > 0 && <details><summary>Ver evidencia publicada</summary><div style={{ overflowX: "auto" }}><table className="data-table"><thead><tr><th>Fuente</th><th>Organismo</th><th>Cargo</th><th>Período</th><th>Monto</th></tr></thead><tbody>{group.evidence.map((item) => <tr key={`${item.kind}:${item.id}`}><td>{item.sourceUrl ? <a href={item.sourceUrl} target="_blank" rel="noopener noreferrer">{item.sourceLabel ?? item.kind} ↗</a> : item.sourceLabel ?? item.kind}</td><td>{item.organization ?? "No informado"}</td><td>{item.role ?? "No informado"}</td><td>{item.period ?? "No informado"}</td><td>{item.amount === null || item.amount === undefined ? "No publicado" : money.format(item.amount)}</td></tr>)}</tbody></table></div></details>}
            </article>)}
          </div>}
          {(pageWindow.totalPages > 1 || hasNext) && <nav className="remuneration-results__pagination" aria-label="Páginas de resultados">
