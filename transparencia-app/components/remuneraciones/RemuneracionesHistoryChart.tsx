...
type ChartMode = "registros" | "montos";
...
export default function RemuneracionesHistoryChart({
  ...
  const [mode, setMode] = useState<ChartMode>("registros");
  ...
  const options = useMemo<EChartsOption>(() => ({
    ...
    legend: {
      ...
      selected: (mode === "registros"
        ? { "Total de registros": true, "Nuevos registros": true, "Registros que ya no aparecen": true }
        : { "Masa bruta publicada": true, "Cambios de monto": true }) as Record<string, boolean>,
    },
    tooltip: {
      trigger: "axis",
    ...
    yAxis: [
      {
        type: "value",
        show: mode === "montos",
        name: mode === "montos" ? "Masa bruta" : "Registros",
      ...
      {
        type: "value",
        name: mode === "registros" ? "Registros" : "Cambios",
        nameTextStyle: { color: tokens.text3, fontSize: 10 },
        axisLabel: { color: tokens.text3, fontSize: 10 },
    ...
    series: mode === "registros"
      ? [
        {
          ...
          areaStyle: { color: tokens.infoBg, opacity: 0.4 },
      ...
      : [
        ...
        { name: "Cambios de monto", type: "bar", yAxisIndex: 1, barMaxWidth: 18, itemStyle: { color: tokens.series[2], borderRadius: [4, 4, 0, 0] }, data: data.map((item) => item.comparison.estado === "comparado" ? item.comparison.cambios : null) },
      ],
  }), [data, labels, mode, selectedIndex, tokens]);
    ...
    <section className="card" aria-labelledby="remuneraciones-evolution-title">
      <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", flexWrap: "wrap", alignItems: "end" }}>
        <div>
          ...
          <p style={{ margin: 0, color: "var(--text-muted)", fontSize: "0.82rem", lineHeight: 1.6 }}>{mode === "registros" ? "Muestra cómo cambia el universo publicado: total de registros, nuevas apariciones y registros que ya no aparecen." : "Muestra la masa bruta mensual y cuántos registros cambiaron de monto frente al mes anterior."}</p>
        </div>
        <div className="segmented-control" aria-label="Cambiar vista de evolución">
          <button type="button" className={mode === "registros" ? "is-active" : ""} aria-pressed={mode === "registros"} onClick={() => setMode("registros")}>Registros</button>
          <button type="button" className={mode === "montos" ? "is-active" : ""} aria-pressed={mode === "montos"} onClick={() => setMode("montos")}>Montos</button>
        </div>
      </div>
      <p style={{ margin: "0.8rem 0 0", color: "var(--text-subtle)", fontSize: "0.74rem" }}>Haz clic en un mes o arrastra el control inferior para explorar. La evolución temporal no prueba por sí sola la causa de un cambio.</p>
