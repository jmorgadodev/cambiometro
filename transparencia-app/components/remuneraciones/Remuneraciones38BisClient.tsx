        ...
        <section className="card" aria-labelledby="release-title">
          <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", flexWrap: "wrap", alignItems: "flex-start" }}>
            ...
            <span className="badge badge-ok">
              {activePeriod.comparison.estado === "comparado"
                ? `Comparado con ${activePeriod.comparison.periodo_anterior}`
                : "Línea base"}
            </span>
          </div>
                    ...
                    <>
                      <div className="table-shell" style={{ marginTop: "0.9rem", overflowX: "auto" }}>
                        <table className="data-table remuneraciones-comparison-table"><caption className="sr-only">{comparisonTitle[comparisonKind]} del corte {activePeriod.mes}</caption><thead><tr><th>Persona</th><th>Organismo y cargo</th><th>Mes anterior</th><th>Mes seleccionado</th><th>Diferencia</th><th aria-label="Acciones" /></tr></thead><tbody>
                          {visibleComparisonRows.map((row) => <tr key={`${row.tipo}-${row.partida}-${row.nombre}-${row.organismo}-${row.cargo}`} className="remuneracion-row" role="button" tabIndex={0} aria-label={`Abrir ficha de ${row.nombre}`} onClick={(event) => { if ((event.target as HTMLElement).closest("button, a")) return; setSelected(row); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelected(row); } }}><td><strong>{row.nombre}</strong><small>{row.partida}</small></td><td>{row.organismo}<small>{row.cargo}</small></td><td>{row.bruto_anterior === null ? "No reportado" : money.format(row.bruto_anterior)}</td><td>{row.bruto_actual === null ? "No reportado" : money.format(row.bruto_actual)}</td><td style={{ fontFamily: "var(--font-mono)", fontWeight: 700, color: row.diferencia === null ? "var(--text-muted)" : row.diferencia >= 0 ? "var(--ok)" : "var(--warn)" }}>{row.diferencia === null ? "—" : `${row.diferencia >= 0 ? "+" : ""}${money.format(row.diferencia)}`}</td><td><button type="button" className="btn btn-ghost" style={{ padding: "0.35rem 0.55rem", fontSize: "0.72rem" }} onClick={() => setSelected(row)}>Ver ficha</button></td></tr>)}
                        </tbody></table>
                      </div>
        ...
        <section className="card" aria-labelledby="explore-title">
          ...
          <div className="table-shell" style={{ marginTop: "1rem", overflowX: "auto" }}>
            <table className="data-table remuneraciones-release-table" style={{ width: "100%" }}>
              ...
              <tbody>
                {visibleRows.map((row) => (
                  <tr key={`${row.partida}-${row.nombre}-${row.organismo}-${row.cargo}`} className="remuneracion-row" role="button" tabIndex={0} aria-label={`Abrir ficha de ${row.nombre}`} onClick={(event) => { if ((event.target as HTMLElement).closest("button, a")) return; setSelected(row); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelected(row); } }}>
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="remuneracion-ficha-title" className="overlay-panel" onClick={() => setSelected(null)}>
          <div className="card" style={{ width: "min(100% - 2rem, 34rem)", margin: "auto", padding: "1.5rem" }} onClick={(event) => event.stopPropagation()}>
            ...
            <section aria-labelledby="remuneracion-historial-title" style={{ marginTop: "1.35rem", paddingTop: "1rem", borderTop: "1px solid var(--border-subtle)" }}>
              ...
              <h3 id="remuneracion-historial-title" style={{ margin: "0.25rem 0 0.3rem", fontSize: "1rem" }}>Historial de montos publicados</h3>
